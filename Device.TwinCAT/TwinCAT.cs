...
namespace Device.Beckhoff
{
    public class TwinCAT : IDeviceHandler
    {
                ...
                tcTask = Task.Run(() =>
                {
                    while (tcTaskRun)
                    {
                        if (DevMode == eDevMode.CONNECT)
                        {
                            ...
                            TwinCATConnector.tcUpdateAxCommand();
                            Thread.Sleep(100);
                        }
                    }
                });
        ...
        public int GET_INT_IN(string id_1, string id_2, string id_3, string id_4, ref bool result)
        {
            ...
            if (index == (int)epInOut.P_IN || index == (int)epInOut.T_IN || index == (int)epInOut.ALARM) 
            {
                int size = int.Parse(id_2);
                int offset = int.Parse(id_3);
            ...
            else if (index == (int)epInOut.D_IN || index == (int)epInOut.D_OUT)
            {
                int size = 1;
                int mod = int.Parse(id_2);
                int set = int.Parse(id_3);
                int num = int.Parse(id_4);

                int offset = (mod * 16 * 16) + (set * 16) + num;

                returnValue = Convert.ToInt32(TwinCATConnector.tcGetData(tcDataType.BOOL, index, offset));
                result = true;
            }
            else if (index == (int)epInOut.M_IN || index == (int)epInOut.M_OUT)
            {
                int size = int.Parse(id_2);
                int axis = int.Parse(id_3);
                int param = int.Parse(id_4);
        ...
        public void SET_INT_OUT(string id_1, string id_2, string id_3, string id_4, int value, ref bool result)
        {
            ...
            else if (index == (int)epInOut.D_OUT)
            {
                int size = 1;
                int mod = int.Parse(id_2);
                int set = int.Parse(id_3);
                ...
                TwinCATConnector.tcSetData(tcDataType.BOOL, index, offset, value);
                result = true;
            }
            else if (index == (int)epInOut.M_OUT)
            {
